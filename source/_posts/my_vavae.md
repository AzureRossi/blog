...
LayerDiffuse 的分布对齐约束提供了一个直接可用的ref -- 把 offset 的均值和方差强制对齐到原始 latent 的统计量上：

$$
\Delta z_{\text{aligned}} = \frac{\Delta z - \mu(\Delta z)}{\sigma(\Delta z)} \cdot \sigma(z_{\text{VAE}}) + \mu(z_{\text{VAE}})
$$

SVG 的 distribution alignment 做的是完全相同的事，只是应用在了不同的地方。在合适的约束下，信息可以被完整地编码进一个分布不变的扰动中，并且可以被准确还原。我们想做的事情，在 INN 的框架下，是被证明可行的。
